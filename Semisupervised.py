...
def run_mnist_pseudo_labeling_app():
        ...
        if info_option == "Tổng quan về ứng dụng và mục tiêu":
            with st.spinner("Đang tải thông tin..."):
                ...
                **Thông tin cơ bản về dữ liệu:**
                ...
                """, unsafe_allow_html=True)
                try:
                    st.image(os.path.join("mhpersoudo", "mnist_overview.png"), caption="Tổng quan về tập dữ liệu MNIST", width=800)
    ...
    with tab_train_eval:
        ...
        if 'split_data' not in st.session_state:
            ...
            """, unsafe_allow_html=True)
            st.info(f"Tham số tối ưu cho {len(X_train_initial)} mẫu: {st.session_state['optimal_params']}")
            ...
            if 'training_results' in st.session_state:
                ...
                if results['loss_history']:
                    ...
                    ax.plot(range(1, len(results['loss_history']) + 1), results['loss_history'], 
                            label='Training Loss (Final)', linestyle='-', color='blue', linewidth=2)
                    ax.set_xlabel("Epochs")
                    ax.set_ylabel("Loss")
                    ax.set_title("Training Loss (Final Iteration)")
                    ax.legend()
                    ax.grid(True)
                    st.pyplot(fig)
                    plt.close(fig)
                ...
                if results['accuracy_history']:
                    ...
                    ax.plot(range(1, len(results['accuracy_history']) + 1), results['accuracy_history'], 
                            label='Training Accuracy (Final)', linestyle='-', color='green', linewidth=2)
                    ax.set_xlabel("Epochs")
                    ax.set_ylabel("Accuracy")
                    ax.set_title("Training Accuracy (Final Iteration)")
                    ax.legend()
                    ax.grid(True)
                    st.pyplot(fig)
                    plt.close(fig)
                ...
                if results['pseudo_labeled_history']:
                    ...
                    ax.plot(range(1, len(results['pseudo_labeled_history']) + 1), results['pseudo_labeled_history'], 
                            label='Số mẫu được gán nhãn giả', linestyle='-', color='purple', linewidth=2)
                    ax.set_xlabel("Vòng lặp")
                    ax.set_ylabel("Số mẫu")
                    ax.set_title("Số mẫu được gán nhãn giả qua các vòng lặp")
                    ax.legend()
                    ax.grid(True)
                    st.pyplot(fig)
                    plt.close(fig)
                ...
                if results['accuracy_test_history']:
                    ...
                    ax.set_xlabel("Vòng lặp")
                    ax.set_ylabel("Accuracy")
                    ax.set_title("Độ chính xác trên tập Test qua các vòng lặp")
                    ax.legend()
                    ax.grid(True)
                    st.pyplot(fig)
                    plt.close(fig)
        ...
        try:
            with st.spinner("Đang tải thông tin huấn luyện..."):
                ...
                if not runs:
                    ...
                    # Biểu đồ Loss (chỉ hiển thị Training Loss của lần huấn luyện cuối cùng)
                    ...
                    if history_metrics:
                        ...
                        ax.set_xlabel("Epochs")
                        ax.set_ylabel("Loss")
                        ax.set_title("Lịch sử Mất mát (Huấn luyện cuối cùng)")
                        ax.legend()
                        ax.grid(True)
                        st.pyplot(fig)
                        plt.close(fig)
                    ...
                    # Biểu đồ Accuracy (chỉ hiển thị Training Accuracy của lần huấn luyện cuối cùng)
                    ...
                    if history_accuracy:
                        ...
                        ax.set_xlabel("Epochs")
                        ax.set_ylabel("Accuracy")
                        ax.set_title("Lịch sử Độ chính xác (Huấn luyện cuối cùng)")
                        ax.legend()
                        ax.grid(True)
                        st.pyplot(fig)
                        plt.close(fig)
                    ...
                    # Biểu đồ số mẫu được gán nhãn giả qua các vòng lặp
                    if 'training_results' in st.session_state and selected_run_id == st.session_state['training_results']['run_id']:
                        results = st.session_state['training_results']
                        if results['pseudo_labeled_history']:
                            fig, ax = plt.subplots(figsize=(10, 5))
                            ...
                            st.info("Không có dữ liệu lịch sử số mẫu được gán nhãn giả để hiển thị.")

                    # Biểu đồ độ chính xác trên tập Test qua các vòng lặp
                    if 'training_results' in st.session_state and selected_run_id == st.session_state['training_results']['run_id']:
                        results = st.session_state['training_results']
                        if results['accuracy_test_history']:
                            fig, ax = plt.subplots(figsize=(10, 5))
                            ...
                            st.info("Không có dữ liệu lịch sử độ chính xác Test để hiển thị.")
